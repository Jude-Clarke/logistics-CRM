...
class DashboardService {
  ...
  async getOverviewStats() {
    ...
    const [statusCounts, totalRevenue, carrierCount] = await Promise.all([
      // 1. Group by status
      Shipment.query().select("status").count("id as count").groupBy("status"),
      // translates to:
      // SELECT "status", count("id") AS "count"
      // FROM "shipments"
      // GROUP BY "status";

      // 2. Sum of all shipment rates
      ...
      // 3. Count of all the carriers (excluding soft-deleted ones)
